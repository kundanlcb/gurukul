...
const Header = () => {
  const { user, currentStudent, switchStudent } = useAuth();
  ...
  const renderStudentItem = ({ item }: { item: Student }) => (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.studentClass}>Class {item.class} • Roll {item.rollNo}</Text>
      </View>
    ...
    <View style={styles.container}>
      <View style={styles.left}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => user.students.length > 1 && setModalVisible(true)}
          disabled={user.students.length <= 1}
        >
